//! MariaDB 11.4 and 11.8 schema semantics and SQL rendering.
...
pub(super) static MARIADB: MariadbProcessor = MariadbProcessor;

pub(super) struct MariadbProcessor;

impl DialectProcessor for MariadbProcessor {
    ...
    fn migration_to_sql(
        ...
        mysql_family::migration_to_sql(FamilyFlavor::Mariadb, migration, start)
    }
    ...
    fn validate_schema(&self, schema: &Schema) -> Result<(), SchemaValidationError> {
        mysql_family::validate_schema(schema, FamilyFlavor::Mariadb)
    }
    fn validate_migration(&self, migration: &Migration) -> Result<(), DialectError> {
        mysql_family::validate_migration(migration, FamilyFlavor::Mariadb)
    }
    fn validate_migration_with_state(
        ...
        mysql_family::validate_migration_with_state(migration, start, FamilyFlavor::Mariadb)
    }
    fn drift_registry(&self) -> &'static crate::drift::DriftRegistry {
        crate::drift::mariadb::registry()
    }
    fn normalize_inspected_schema(&self, schema: Schema) -> Result<Schema, SchemaValidationError> {
        schema.prepare(crate::dialects::Dialect::Mariadb)
    }
    fn column_for_repair(&self, expected: &Column, observed: &Column) -> Column {
        mysql_family::column_for_repair(expected, observed, FamilyFlavor::Mariadb)
    }
