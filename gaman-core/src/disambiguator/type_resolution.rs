...
use crate::disambiguator::ids::{clarification_id, is_unknown_type_id};
use crate::disambiguator::{
    Answer, Clarification, ClarificationKind, Decision, DisambiguatorError, Severity,
};
...
pub fn resolve_unknown_types(
    ...
    previous: &Schema,
    decisions: &[Decision],
) -> Result<TypeResolution, DisambiguatorError> {
...
fn validate_type_decisions(
        ...
        .filter(|decision| is_unknown_type_id(&decision.clarification_id))
    {
        ...
        let clarification = by_id.get(&decision.clarification_id).ok_or_else(|| {
