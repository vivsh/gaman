...
impl Disambiguator {
    pub fn process(
        ...
        DisambiguationPlan::new(ops).process(ops, decisions)
    }
}
...
mod tests {
    ...
    fn decisions_rewrite_rename_and_backfill_operations() {
        ...
        let resolved = match Disambiguator.process(&ops, &decisions).unwrap() {
            DisambiguationResult::Resolved(ops) => ops,
            DisambiguationResult::NeedsInput(c) => panic!("unexpected clarification: {c:?}"),
        };
    ...
    fn partial_decisions_return_only_still_pending_clarifications() {
        ...
        let pending = match Disambiguator
            .process(
                &ops,
    ...
    fn duplicate_decision_ids_fail() {
        ...
        assert_eq!(
            err,
            DisambiguatorError::DuplicateDecision("rename_col:t:x".to_string())
        );
    }
    ...
    fn conflicting_rename_targets_fail_before_rewrite() {
        ...
        assert!(matches!(
            err,
            DisambiguatorError::ConflictingRenameTarget { .. }
        ));
    }
    ...
    fn empty_default_or_cast_input_fails() {
        ...
        let err = Disambiguator
            ...
            .unwrap_err();
        assert!(matches!(err, DisambiguatorError::EmptyInput { .. }));
    ...
    fn invalid_decision_errors_are_preserved() {
        ...
        let invalid_candidate = Disambiguator
            .process(
                &ops,
                &[decision(
                    "rename_col:t:x",
        ...
        assert!(matches!(
            invalid_candidate,
            DisambiguatorError::InvalidCandidate { .. }
        ));

        let invalid_answer = Disambiguator
            .process(
                &ops,
                &[decision(
                    "rename_col:t:x",
        ...
        assert!(matches!(
            invalid_answer,
            DisambiguatorError::InvalidAnswer { .. }
        ));
    }
}
